...
const SearchComplaints = () => {
    ...
    const fetchComplaints = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8000/complaints/studentdashboard"
        );
  ...
  const markAsDone = async (id) => {
    if (
      window.confirm("Are you sure you want to mark this complaint as Done?")
    ) {
      try {
        await axios.put(`http://localhost:8000/complaints/markAsDone/${id}`);
        setComplaints(
          complaints.map((complaint) =>
            complaint._id === id ? { ...complaint, status: "Done" } : complaint
          )
        );
      } catch (error) {
  ...
  const markAsInProgress = async (id) => {
    try {
      await axios.put(
        `http://localhost:8000/complaints/markAsInProgress/${id}`
      );
      setComplaints(
        complaints.map((complaint) =>
          complaint._id === id
  ...
  const deleteComplaint = async (id) => {
    try {
      await axios.delete(
        `http://localhost:8000/complaints/deleteComplaint/${id}`
      );
  ...
  const filteredComplaints = complaints.filter(
    (complaint) =>
      ...
      complaint.studentId.toLowerCase().includes(searchStudent.toLowerCase())
  );
        ...
        <Card.Body style={{ width: "70%" }}>
          ...
          <div className="form-group mt-3">
            ...
            >
              ...
              <option value="1. Safety and Security">
                Safety and Security
              </option>
              <option value="2. Maintenance and Repairs">
                Maintenance and Repairs
              </option>
              <option value="3. Facilities and Amenities">
                Facilities and Amenities
              </option>
              <option value="4. Hygiene and Cleanlines">
                Hygiene and Cleanliness
              </option>
              <option value="5. Booking and Payments">
                Booking and Payments
              </option>
              <option value="6. Food and Dining">Food and Dining</option>
              <option value="7. Staff and Management">
                Staff and Management
              </option>
              <option value="8. Room Assignments and Accommodation">
                Room Assignments and Accommodation
              </option>
                        ...
                        <Row className="align-items-center">
                          ...
                          <Row>
                            ...
                            <Button
                              style={{ width: "33%" }}
                              variant="warning"
                              onClick={() => markAsInProgress(complaint._id)}
                            >
                              In Progress
                            </Button>
                            <Button
